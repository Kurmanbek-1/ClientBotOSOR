...
async def load_contact(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await message.answer('Введите Адрес', reply_markup=buttons.cancel_markup)
    await TryonFSM.next()
        ...
        for i in Manager:
            if message.text.lower() == 'да':
                ...
                await message.answer('Отлично!\n'
                                     'Ваш примерка отправлена менеджерам ✅\n'
                                     'Пожалуйста ожидайте с вами скоро свяжутся ⏳', reply_markup=buttons.start)

                await bot.send_message(chat_id=i, text=f"📢Новый заказ на примерку!\n"
...
def register_try_on(dp: Dispatcher):
    ...
    dp.register_message_handler(load_size, state=TryonFSM.size)
    dp.register_message_handler(load_submit, state=TryonFSM.submit)
